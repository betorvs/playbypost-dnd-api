...
func Character(c echo.Context) (err error) {
	...
	if err != nil {
		errString := fmt.Sprintf("Cannot calculate character post %v", err)
		return c.JSON(http.StatusBadRequest, utils.FormatMessage(errString))
	}
...
func CheckKnownSpellList(c echo.Context) (err error) {
	...
	if !utils.StringInSlice(class, usecase.ClassWithSpellKnown()) {
		errString := "your class dont have known spell list"
		return c.JSON(http.StatusBadRequest, utils.FormatMessage(errString))
	}
