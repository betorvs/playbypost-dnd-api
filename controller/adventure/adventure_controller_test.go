...
func TestGetOneAdventure(t *testing.T) {
	...
	if assert.NoError(t, GetOneAdventure(c2)) {
...
func TestPostAdventure(t *testing.T) {
	...
	example4 := adventure.Adventure{}
	example4.CampaignID = "5e70e4c5d2f4f777c16b29f6"
	example4.Status = "BLA"
	...
	c4.SetPath("/adventure")

	// Assertions
	if assert.NoError(t, PostAdventure(c4)) {
...
func TestPutAdventure(t *testing.T) {
	...
	example3 := adventure.Adventure{}
	example3.CampaignID = "5e70e4c5d2f3f777c16b29f8"
	example3.Status = "onhold"
	requestByte3, _ := json.Marshal(example3)
	requestReader3 := bytes.NewReader(requestByte3)
	...
	example4 := adventure.Adventure{}
	example4.CampaignID = "5e70e4c5d2f3f777c16b29f6"
	...
	// Assertions
	if assert.NoError(t, PutAdventure(c4)) {
...
func TestChangeAdventureStatus(t *testing.T) {
	...
	rec3 := httptest.NewRecorder()
	c3 := e.NewContext(req3, rec3)
	c3.SetPath("/adventure/:id/:status")
	c3.SetParamNames("id", "status")
	c3.SetParamValues("BLA", "living")

	// Assertions
	if assert.NoError(t, ChangeAdventureStatus(c3)) {
		assert.Equal(t, http.StatusBadRequest, rec3.Code)
	}
	...
	rec4 := httptest.NewRecorder()
	c4 := e.NewContext(req4, rec4)
	c4.SetPath("/adventure/:id/:status")
	c4.SetParamNames("id", "status")
	c4.SetParamValues("5e70e4c5d2f3f777c16b29f6", "living")

	// Assertions
	if assert.NoError(t, ChangeAdventureStatus(c4)) {
		assert.Equal(t, http.StatusBadRequest, rec4.Code)
	}
}
